(function(window){
    ...
    Seed.prototype = {
        ...
        drawText: function() {
            ...
            ctx.moveTo(0, 0);
            ctx.scale(0.75, 0.75);
            ctx.font = "8px 微软雅黑,Verdana";
            ctx.fillText("Nhấn vào trái tim", 23, 10);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
        ...
        hover: function(x, y) {
            // Cheap bounding-box hit test instead of ctx.getImageData (expensive)
            var cirle = this.cirle;
            var point = cirle.point;
            var scale = cirle.scale;
            var radius = cirle.radius * scale;
            var left = point.x - radius;
            var right = point.x + radius;
            var top = point.y - radius;
            var bottom = point.y + radius;
